...
//    static int mask = ( 1 << (hashlength) ) -1;
    ...
    //static assert, used as assertion in compiling
    static_assert(
        sizeof(keyType)==4
        || sizeof(keyType)==8
        || sizeof(keyType)==12
...
public:
    valueType * m;
    uint8_t * Z1; // this array is the array D1 used in FIB update and concurrency control 
    uint8_t * Z2;
    uint32_t s1, s2; // it seems to be used to select hash funtion 
    uint8_t *filled; // what exactly does filled array used for ?  seems to be used in dfs process to judge if the relativve position has already been filled
    uint32_t hashshr1, mask1;
    uint32_t hashshr2, mask2, m2shift;
private:
    keyType *keys;// only exist in control structures while in query structures ,this is assigned as NULL
    valueType *values;

    // first array should be used in dfs process in which 
    // first[i] indicate the first elelment that 
    // belongs to connected component whhich contains i
    // while next1 and next2 must have something to do with first array
    ...
    uint32_t *fa; // used as a union-find set to avoid cyclic
...
private:
    void findnewhash()  //called by construction function
    {
        s1 = rand();
        s2 = rand();

	//clean existing data for new hash construction
        memset(m,0,sizeof(valueType)* othelloHashUpperBound);
        memset(Z1,0,1 << COUNTERLENGTH); // initialized as zero 
        memset(Z2,0,1 << COUNTERLENGTH);
        memset(filled, 0, keycountPreserve>>3);
        trycount++;
    }

    //to get what connected component does k belong to
    uint32_t getfa(uint32_t k) 
    {
        if (fa[k] & 0xF0000000) return fa[k] = k;
	//recursion until fa[k] = k
    ...
    {
        fa[getfa(k1)] = getfa(k2); // k2 is k1's father
    }

    //this is used to make bigger key array and other neccessities to uh
...
public:
    Othello(keyType * _keys, valueType *_values, uint32_t keycount);// construction function --- according to original map of keys and values
    Othello(valueType * _m, uint8_t *Z1, uint8_t *Z2, uint32_t _s1, uint32_t _s2, uint32_t _mask1, uint32_t _mask2); // construction function used by different threads

    // print keys and values. This function should be used to debug
    void printkeys() 
    {
	...
	// s1 could be set in findnewhash() funtion
	// hashshr1 and mask1 have been set in contruction function 
...
//corresponds to susceptibility in complexity analysis of name addition or something else 
template<typename keyType, typename valueType>
...
//called by modifyValuesToDesire process
//parameter root: h2 flip: m[h1]^m[h2]^newValue grandpa:h1
template<typename keyType, typename valueType>
...
// called by batchupdate function
template<typename keyType, typename valueType>
bool Othello<keyType, valueType>::keyexists(keyType k, bool &addsucc , valueType v, bool tryadd) {
    //examine whether k exists in the graph, if not,
    //try to (if tryadd == true) add a key into graph, but not modify the values,
...
// this function seems now is useless
// firstly, updateBuffer is empty
// and secondly, according to testOthelloUpdate.cc 
// all m, Z1, Z2 of these friends refer to centralized othello
// before notifyfriends function is called,
// m, Z1, Z2 in centralized othello have already been modified 
// In a nutshell, this is useless
template<typename keyType, typename valueType>
...
//            printf("::%d:: "            ,i);
#endif
        bool keyexist = keyexists(k[i], addsucc,v[i],true); // to  make sure that the keys already exists in keyset
        if ((!keyexist) && !(addsucc)) { 
...
bool Othello<keyType, valueType>::batchupdateOrInsert(keyType *k, valueType *v, uint32_t cnt, bool skipNewkeyCheck) {
    ...
    // if batchupdate return true, this means that
    // there exsit cycle
    ...
    keycount = bkkeycount;// recover keycount
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        if (!skipNewkeyCheck)
            addthiskey = !keyexists(k[i],tmpbool,tmpv,false); // do not add key to keys array 
        if (addthiskey) {
            if (keycount * 2>= keycountPreserve) {
                growkeycountPreserve();
            }
...
valueType Othello<keyType,valueType>::query(const keyType &k) {
	...
	// the procedure of query should be executed in the given explicit order
	// this can be specified by compiler reorder barriers on strong memory model
	// such as x86_64, or fence instructions on weak memory model platform such as ARM
        __asm__ __volatile__ ("" ::: "memory");
...
Othello<keyType,valueType>::Othello(keyType * _keys, valueType *_values, uint32_t _keycount) 
{
    int hl1 = 0;
    int hl2 = 0 ;
    while ((1<<hl1) < _keycount* 1.333334) hl1++; //hash length of ma
    while ((1<<hl2) < _keycount * 1) hl2++; // hash length of mb
    mask1 = (1<<hl1) -1;
    mask2 = (1<<hl2) -1;

    //hashshr1 and hashshr2 is used in get_hash() function
    //but I know little about the function of hashshr1(2)
    //maybe a necessity of specific hash function ? 
    hashshr1 = hl1 ^ ( 7 & s1);
    hashshr2 = hl2 ^ ( 7 & s2);

    // m2shift is used to decipt the size of ma,
    // since ma and mb are allocated memory together
    // their space is constant,
    // so it is necessary to distinguish the two 
    // which is exactly the role m2shift is playing
    // This is obvious by looking at get_hash_2 function
    m2shift = mask1+1;

    keycount = _keycount;
//    keycountPreserve = (1<<HASHLENGTH) * 3 / 2;
    keycountPreserve = keycount * 2; // preserved for more keys
    othelloHashUpperBound = m2shift + mask2 + 1; // = 1<<hl1 + 1<<hl2

#ifndef NO_OTHELLO_INFO
    printf("HASHLENGTH %d %d\n", hl1,hl2);
#endif

    // what is happening here ? it seems that it is related to time XXX
    tm1.tv_sec = 0;
    tm1.tv_nsec = 5;

    //allocate memory space for bigraph
    m = (valueType *) valloc(othelloHashUpperBound* sizeof(valueType));

    // Z1 and Z2 is used in update and concurrency control
    // a bit index in Z1 or Z2 corresponds to multiple elements
    Z1 = (uint8_t *) valloc(sizeof(uint8_t)  << COUNTERLENGTH);
    Z2 = (uint8_t *) valloc(sizeof(uint8_t)  << COUNTERLENGTH);

    //why is filled array's space is smaller than I have expected ? XXX
    ...
    //allocate memory space for keys and values , used in control structure
...
#else
    printf("Building Othello %d Keys, %d nodes each side, keysize %d, valuesize %d \n",keycount, othelloHashUpperBound >> 1, sizeof(keyType),sizeof(valueType) );
#endif
    while ((!builtsucc)&&(trycount < MAX_REHASH)) 
    {
...
//another kind of construction function used to synchronize to threads
template<typename keyType, typename valueType>
Othello<keyType,valueType>::Othello(valueType * _m, uint8_t * _Z1, uint8_t * _Z2, uint32_t _s1, uint32_t _s2, uint32_t _mask1, uint32_t _mask2) {
    ...
    keys = NULL; // keys arrya is useless in threads
}

//called by construction function
template<typename keyType, typename valueType>
bool Othello<keyType,valueType>::trybuild(keyType *keys, valueType *values, uint32_t keycount) 
{
    //set to all 1, why? used for AND 0xf0000000 ?  to be judged if the exact position has been taken 
    ...
    {
        keyType kk = keys[i];
	// get hash function according to keys
	// the value of h1 and h2 is set by reference
	// the value of h1 and h2 is the rank of m
        get_hash(kk, h1, h2);
#ifdef TEST
        printf("%d %llx %x %x :%x %x\n",i,kk,h1,h2,s1,s2);
#endif

	// to check if h1 and h2 belong to the same connected component
        if (getfa(h1) == getfa(h2)) return false;
        merge(h1,h2);


	// first and next1 and next2 seems like a cycle link
    ...
    //computing bitmaps?
    //in this sense, dfs seems to be used to fill or calculating bitmaps
    memset(filled,0,(keycountPreserve >> 3) +1);
    for (uint32_t i = 0; i < keycount; i++) {
	// why is here part after & operator, so called filledbit, is necessary ? XXX
