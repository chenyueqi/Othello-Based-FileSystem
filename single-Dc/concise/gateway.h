...
{
    if(!op.compare("ls") || !op.compare("write") || !op.compare("read") || !op.compare("rm") || !op.compare("cp") || !op.compare("touch") || !op.compare("mv"))
	return sendMessageToServer(op, path1, path2, id1, id2);
    else if(!op.compare("mkdir") || !op.compare("mvr") || !op.compare("rmr"))
 ...
 */
bool Gateway::sendMessageToServer(const string op, const string path1, const string path2, const uint64_t id1, const uint64_t id2)
{
    if(!op.compare("touch"))
